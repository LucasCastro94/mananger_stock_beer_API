...
import lucascastro.git.com.Mananger_Beer_API.builder.BeerDTOBuilder;
...
import static lucascastro.git.com.Mananger_Beer_API.utils.JsonConvertionUtils.asJsonString;
...
public class BeerControllerTest {
    ...
    void whenGETListWithBeersIsCalledThenOkStatusIsReturned() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get(BEER_API_URL_PATH)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect((ResultMatcher) jsonPath("$[0].name", is(beerDTO.getName())))
                .andExpect((ResultMatcher) jsonPath("$[0].brand", is(beerDTO.getBrand())))
                .andExpect((ResultMatcher) jsonPath("$[0].type", is(beerDTO.getType().toString())));
    }
    ...
    void whenPATCHIsCalledToIncrementDiscountThenOKstatusIsReturned() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.patch(BEER_API_URL_PATH + "/" + VALID_BEER_ID + BEER_API_SUBPATH_INCREMENT_URL)
                ...
                .andExpect((ResultMatcher) jsonPath("$.brand", is(beerDTO.getBrand())))
                .andExpect((ResultMatcher) jsonPath("$.type", is(beerDTO.getType().toString())))
                .andExpect((ResultMatcher) jsonPath("$.quantity", is(beerDTO.getQuantity())));
    }
